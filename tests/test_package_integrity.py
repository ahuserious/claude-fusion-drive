...
from claude_fusion_drive import __version__ as fusion_drive_version
from relentless_inception import __version__ as inherited_engine_version
...
class PluginPackageIntegrityTests(unittest.TestCase):
    ...
    def test_release_identity_is_consistent_across_install_and_runtime_surfaces(self) -> None:
        ...
        self.assertEqual(fusion_drive_version, "0.1.0")
        ...
        self.assertEqual(
            headers["User-Agent"],
            f"relentless-inception-codex/{inherited_engine_version}",
        )
    ...
    def test_mcp_manifest_uses_codex_camel_case_and_resolves_server(self) -> None:
        ...
        self.assertEqual(server["args"], ["${CLAUDE_PLUGIN_ROOT}/mcp_server.py"])

        self.assertNotIn("cwd", server)  # Claude Code resolves ${CLAUDE_PLUGIN_ROOT}; no cwd contract exists.
        resolved_argument = server["args"][0].replace("${CLAUDE_PLUGIN_ROOT}", str(PLUGIN_ROOT))
        server_path = Path(resolved_argument).resolve()
    ...
    def test_marketplace_source_resolves_to_manifest_plugin(self) -> None:
        ...
        marketplace_plugin_root = self._resolve_inside(REPOSITORY_ROOT, source)
        self.assertEqual(marketplace_plugin_root, PLUGIN_ROOT.resolve())
        self.assertTrue((marketplace_plugin_root / ".claude-plugin" / "plugin.json").is_file())
    ...
    def test_native_grok_role_keeps_instructions_at_root_and_documents_valid_mcp_overrides(self) -> None:
        ...
        self.assertEqual(default_config["native_claude"]["reviewer_roles"], [])
        self.assertEqual(default_config["native_claude"]["reasoning_only_roles"], [])
    ...
    def test_shipped_maximum_intelligence_defaults_are_frontier_only(self) -> None:
        ...
        native_claude = default_config["native_claude"]
        self.assertEqual(native_claude["executor_model"], "gpt-5.6-sol")
        self.assertEqual(native_claude["reviewer_models"], ["gpt-5.6-sol"])
