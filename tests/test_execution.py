...
class ExecutionHandoffTests(unittest.TestCase):
    def test_handoff_selects_sections_and_waits_for_host_owned_pre_mutation_gates(self) -> None:
        ...
        self.assertEqual(handoff["execution_contract"]["native_claude"]["executor_model"], "gpt-5.6-sol")
        self.assertTrue(handoff["execution_contract"]["native_claude"]["require_gate_after_execution"])
    ...
    def test_recursive_cli_uses_only_hash_bound_persisted_settings(self) -> None:
        execution = _execution_config(
            mode="codex_cli",
            allow_recursive_claude_cli=True,
    ...
    def test_recursive_cli_refuses_packet_with_pending_host_gates(self) -> None:
        execution = _execution_config(mode="codex_cli", allow_recursive_claude_cli=True)
        handoff = build_handoff(
            "Plan",
            "run-pending",
