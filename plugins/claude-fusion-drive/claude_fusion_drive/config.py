...
def runtime_dir() -> Path:
    ...
    path = Path(configured).expanduser() if configured else Path.home() / ".claude" / "claude-fusion-drive"
...
def load_config(*, include_user: bool = True, validate: bool = True) -> dict[str, Any]:
    ...
    if validate:
        ...
        if errors:
            raise ConfigurationError("Invalid Claude Fusion Drive configuration:\n- " + "\n- ".join(errors))
    return config
...
def validate_config(config: Mapping[str, Any]) -> list[str]:
    ...
    if not isinstance(lifecycle, Mapping) or not lifecycle.get("require_claude_goal_before_execution"):
        errors.append("lifecycle must require a Claude goal before execution")
    if (
