...
def runtime_data_dir() -> Path:
    ...
    return Path.home() / ".claude" / "claude-fusion-drive" / "engine"
...
def validate_config(config: Mapping[str, Any]) -> List[str]:
    ...
    for profile_name, profile in profiles.items():
        ...
        if isinstance(execution, Mapping):
            ...
            if execution.get("allow_recursive_claude_cli") is True and execution.get("mode") != "codex_cli":
                errors.append(
                    f"{path}.execution.allow_recursive_claude_cli may be true only when mode='codex_cli'"
                )
