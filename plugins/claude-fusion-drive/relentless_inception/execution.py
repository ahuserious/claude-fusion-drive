...
def _instruction(
    ...
    *,
    ...
    if pending_gates:
        authorization = (
            "Do not mutate files or external state yet. The active Claude host must independently invoke "
            + ", ".join(pending_gates)
    ...
    return (
        "This is a persisted Claude host-workflow packet, not independent permission to act. "
...
def build_handoff(
    ...
    *,
    ...
    active Claude host and are never silently inferred from the synthesis verdict.
    """
...
def execute_codex_cli(
    ...
    *,
    ...
    if execution.get("allow_recursive_claude_cli", False) is not True:
