...
- **Plugin providers** call external APIs from the bundled MCP server and return text or structured deliberation artifacts. These seats have no host workspace tools.
- **Native Codex providers** are optional user-level configuration for a separately operated Codex CLI, inherited from the validated Codex edition. When compatible, a model can participate in Codex's tool loop under Codex's sandbox. On this Claude Code edition the Codex CLI is not the host; every "native Codex" section below is inherited lineage documentation, and native delegation on Claude Code uses Claude subagents.

Configure the first with the `claude-fusion-drive-config` skill and MCP tools. Configure the second manually and only with explicit consent.

## Plugin provider configuration

The merged plugin configuration comes from:

1. the shipped `plugins/claude-fusion-drive/config/default.json`;
2. an optional user override at `RELENTLESS_INCEPTION_CONFIG`, or otherwise `PLUGIN_DATA/config.json` / `~/.claude/claude-fusion-drive/engine/config.json`.
...
Use the exact API id `grok-4.5` for direct xAI seats. `grok-4.5-latest` is the native Grok Build alias used by the companion plugin's subagents; this plugin's MCP runtime deliberately pins the exact API id. Shipped defaults never fall back to an older Grok model.
...
### Native Codex through OpenRouter (inherited Codex-edition lineage)
...
The plugin's direct OpenAI seat is independent from any ChatGPT subscription used by the codex CLI. API billing and retention follow the API account, not a CLI login.
...
## Native Codex provider setup (inherited Codex-edition lineage)

This section and its subsection document the upstream Codex CLI host from the validated Codex edition; they apply only to a separately operated Codex CLI, not to the Claude Code host.
