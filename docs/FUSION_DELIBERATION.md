# Fusion Deliberation in Claude Code

Relentless Inception turns a difficult host task into an evidence-bearing sequence of independent proposals, structured comparison, fresh synthesis, exact-artifact review, and host-controlled execution. The runtime supplies the control plane; the Claude Code session remains the only default surface with workspace authority.
...
1. **Map** — the Claude Code host supplies the goal, constraints, evidence, deterministic checks, and explicit context boundaries. External seats never discover the workspace on their own.
...
6. **Execute** — the runtime produces a hash-bound packet. The active Claude Code session must still complete enabled plan and pre-execution gates before mutation, then owns implementation, approvals, tests, diff review, and later lifecycle gates.

## Maximum-intelligence topology

This engine-level profile is opt-in in this edition; the control-plane default active profile is `xai-claude-oauth` (see [FUSION_DRIVE_ARCHITECTURE.md](FUSION_DRIVE_ARCHITECTURE.md)).

| Role | Shipped selection | Authority |
|---|---|---|
| Active host and execution handoff | Claude Code session on `claude-fable-5` | Workspace, tools, approvals, implementation, tests |
...
There is no automatic weaker-model fallback in `maximum_intelligence`. A missing required seat, schema-invalid output, unknown blocking cost, panel collapse, or failed gate stops the run visibly. Grok-only external seats are role-diverse multi-agent deliberation; cross-model fusion requires a separately configured model family. The claude-fable-5 host does contribute a different family to the overall plan-and-execute workflow, but its host reasoning is not falsely counted as an external panel receipt.
...
The following captures come from the original Claude-hosted ancestor edition. They document the user-facing mental model that this runtime preserves, not the current UI or default model names.
...
For this Claude Code edition, translate those labels as follows:

| Original capture | Current runtime equivalent |
|---|---|
| Claude Code session | active Claude Code `claude-fable-5` host |
...
Each stage is host-invoked because only the Claude Code host can collect local evidence. The MCP runtime enforces the reviewer roster, quorum, hash identity, structured verdicts, amendment bounds, receipts, and budget state. The Claude Fusion Drive control plane above the engine additionally records `synthesis`, `subagent_pre_execution`, and `subagent_post_execution` receipts for its canonical eight-stage lifecycle.
