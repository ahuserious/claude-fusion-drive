...
External API panelists are not native Claude Code subagents. They receive only the explicit task/context supplied to the fusion tool and optional provider-hosted tools. The active Claude Code host owns workspace inspection, writes, commands, and approvals. This boundary is structural, not prompt-only.

## Known boundaries

- A plugin cannot install or select host-level provider configuration or custom agents through its manifest. The inherited native-Codex agent/provider TOMLs remain opt-in templates for a separately operated Codex CLI and must be smoke-tested on that installed Codex version.
