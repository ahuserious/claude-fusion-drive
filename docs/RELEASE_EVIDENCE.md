...
Every observation on this page was produced by the validated Codex edition (`codex-fusion-drive`, inherited engine 0.1.4) with a Codex CLI host. It is retained as inherited upstream evidence. The Claude Code edition (0.1.0) has not re-run the Terminal-Bench, DeepSWE, or live direct-xAI campaigns, and no new benchmark results are claimed for this edition; the offline test suite is the only evidence re-run here.
