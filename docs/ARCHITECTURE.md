...
This page documents the vendored inherited engine (`relentless_inception`). Its host in this edition is Claude Code; sections that describe the upstream Codex host runtime are marked as inherited lineage documentation.

Relentless Inception is the deliberation engine inside the Claude Code plugin. It separates **deliberation** from **execution**. A local MCP server can ask several external models for independent analyses, compare them, produce a generative synthesis, and gate that synthesis. The active Claude Code session then decides whether and how to execute the resulting handoff under the user's normal workspace permissions.
...
```mermaid
flowchart LR
    U["User and active Claude Code session"]
    S["Plugin skills"]
    ...
    H["Execution handoff"]
    W["Claude Code tools, sandbox, approvals, and workspace"]
    N["Optional native Claude Code subagents"]
    ...
    H --> U
    U --> W
    U -. "optional host delegation" .-> N --> W
```

The arrow from the handoff returns to the active Claude Code session. It does not continue from the MCP server into the workspace.

## Components

### Claude Code plugin package

The installable plugin lives under `plugins/claude-fusion-drive/`:

- `.claude-plugin/plugin.json` provides plugin metadata and points Claude Code to skills and MCP configuration.
...
- `examples/native-codex-*.toml.example` contains opt-in setup snippets inherited from the Codex edition; they apply only to a separately operated Codex CLI.

Claude Code plugins can bundle skills and MCP servers, but the current plugin manifest does not install arbitrary native subagents or host-level provider configuration. Native delegation on Claude Code uses ordinary Claude Code subagents under the host's own settings.
...
The configuration schema is intentionally broader than one Python function because several controls belong to the Claude host:
...
| Host skills and active session | Goal/scope confirmation, path selection and redaction before egress, acceptance criteria, mechanical evidence collection, active-workspace execution, approvals, diff review, and post-execution submission. |
| Native Claude Code runtime | Actual subagent availability, model selection, inherited tools, sandbox, permission mode, and native tool-loop compatibility. |
| Provider/router | Upstream retention, model availability, routing, usage reporting, and remote cancellation behavior. |

A setting in `native_claude`, `privacy`, `evidence`, or `execution` does not grant the MCP process host permissions. It gives the host skill an explicit policy to follow and audit.
...
The MCP server stores run artifacts beneath `PLUGIN_DATA` when the host supplies it, `RELENTLESS_INCEPTION_DATA_DIR` when explicitly set, or `~/.claude/claude-fusion-drive/engine/` as the fallback. A run directory is keyed by a generated run id and binds:
...
The active Claude Code session constructs a bounded packet containing the task, acceptance criteria, constraints, and the smallest sufficient evidence bundle. All artifact content is fenced as untrusted data.
...
5. applies repository instructions and current user scope, requesting approvals through Claude Code when needed;
6. edits with ordinary Claude Code tools and runs mechanical checks;
...
| Relentless Inception MCP server | Only explicit local config/run data and content sent by the host | Its own bounded provider/state operations | No interactive host approval authority | Only plugin/run configuration and state |
| Active Claude Code session | Yes, within sandbox | Yes | Yes | According to sandbox and user scope |
| Native Claude Code subagent | According to inherited tools and sandbox | Yes | Only where the runtime can surface approval; otherwise approval-required actions fail | According to inherited/effective sandbox |

This distinction is central. Describing an API panelist as a "Grok subagent" would be misleading. Native Claude Code subagents run Claude models under the host's own tool loop; Grok participates only through the plugin's external seats.

## Optional native Codex agents (inherited Codex-edition lineage)

This section is retained from the validated Codex edition. It documents the upstream Codex 0.145 host runtime, not the Claude Code host; it applies only to a separately operated Codex CLI. Native delegation on Claude Code uses ordinary Claude Code subagents.
...
- This release does not promise an always-running watchdog or unattended daemon. Continued host execution still depends on the active task and its normal lifecycle.

## Design lineage

This implementation keeps the strongest ideas from the user's prior systems while dropping host-specific assumptions:

| Source | Preserved idea | Adaptation |
|---|---|---|
| Claude/Grok Relentless Inception | Plan/phase/summary gates, persistent evidence, rescue-oriented checkpoints | MCP run state plus explicit active-host handoffs; no unverified TUI relay or hook claim. |
| Batch Create Eval | Decomposition, per-unit verification, simulated-user shakedown | The active host session owns work units and real tests. |
...
- External seats do not browse, use MCP tools, run shell commands, or inspect files unless the active Claude Code session first supplies the resulting evidence.
...
- Plugin install does not install API keys, host-level provider configuration, or custom agents.
