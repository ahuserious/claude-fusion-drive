...
PLUGIN_ROOT = BENCH_ROOT.parent / "plugins" / "claude-fusion-drive"
...
FUSION_TASK_PREFIX = (
    "Produce a pre-execution plan for the active Claude host to fulfill the exact "
...
def _expected_handoff_instruction(
    ...
    authorization = (
        "Do not mutate files or external state yet. The active Claude host must "
    ...
    completion = (
        " After implementation, the active Claude host must invoke the configured "
        + ", ".join(later_gates)
        + " gate(s) over the exact resulting artifact and evidence."
    )
    return (
        "This is a persisted Claude host-workflow packet, not independent permission "
...
def validate_contract(contract: dict[str, Any], index: dict[str, Any]) -> None:
    ...
    require(
        observed.get("ri_data_directory") == "/logs/agent/claude-fusion-drive",
    ...
    plugin_mount = mount_by_target.get("/opt/claude-fusion-drive")
    ...
    require(
        str(plugin_mount.get("source", "")).endswith("/plugins/claude-fusion-drive"),
        "Bad plugin source mount",
    )
    support_mount = mount_by_target.get("/opt/claude-fusion-drive-bench")
    ...
    secret_mount = mount_by_target.get("/run/secrets/claude-fusion-drive-xai")
...
def is_relentless_inception_item(item: Any) -> bool:
    ...
    return server == "claude-fusion-drive"
...
def validate_codex_log(path: Path, trajectory: Any) -> dict[str, Any]:
    ...
    require(
        ...
        and "active Claude host" in fusion_context
        and "not an external seat" in fusion_context
        and "isolated from this workspace" in fusion_context,
        "Fusion context does not bind execution to the active Claude host",
    )
    ...
    for (_, command_item), source in zip(execution_commands, execution_sources):
        require(
            command_is_bound_to_wrapper(command_item.get("command"), source),
            "Claude host-execution command differs from its ATIF wrapper",
        )
    ...
    for source, decoded_output, (_, completed_command) in zip(
        ...
        require(
            ...
            "Claude host-execution output is not a retained suffix of its ATIF transcript",
        )
    ...
    for stage, expected_run_id in EXPECTED_RUN_IDS.items():
        ...
        if stage == "fuse":
            ...
            expected_artifacts_dir = (
                f"/logs/agent/claude-fusion-drive/runs/{expected_run_id}"
            )
            require(
            ...
            require(
                lifecycle.get("stage_owner") == "claude_host"
