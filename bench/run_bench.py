...
def verify_artifact_hashes(pins: dict[str, Any]) -> dict[str, str]:
    observed = {
        "plugin_tree_sha256": tree_sha256(REPOSITORY_ROOT / "plugins" / "claude-fusion-drive"),
...
def build_command(
    ...
    plugin_source = REPOSITORY_ROOT / "plugins" / "claude-fusion-drive"
    benchmark_support = BENCH_ROOT / "support"
    resolved_secret_file = secret_file or Path(
        "/private/tmp/claude-fusion-drive-xai"
    )
    tool_mounts = [
        {
            "type": "bind",
...
def resolve_xai_api_key() -> str:
    ...
    plugin_root = REPOSITORY_ROOT / "plugins" / "claude-fusion-drive"
...
def build_evidence_index(attempt_directory: Path, contract: dict[str, Any]) -> Path:
    ...
    ri_root = trial_directory / "agent" / "claude-fusion-drive" / "runs"
    ri_runs = []
